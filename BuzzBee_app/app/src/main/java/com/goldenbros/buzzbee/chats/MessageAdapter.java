...
public class MessageAdapter extends BaseAdapter {
    ...
    public void addMessage(WritableMessage message, int direction) {
        Log.i(message.getTextBody(), messages.size()+"");
        if(messages.size()!=0){
            String lastOne = messages.get(messages.size()-1).first.getTextBody();
            if(message.getTextBody().equals(lastOne)){
    ...
    @Override
    public int getItemViewType(int i) {//check incoming or outgoing
